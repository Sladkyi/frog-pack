...
- Each role also counters something (`ROLES[].counter`, shown on the item card and as a colored tag on the attack button): Nova is best against packs; Burst ignores armor and breaks boss wind-ups twice as fast; Vortex slows boss wind-ups and attacks; Blight stops blighted foes from healing (regen, healers, boss MEND) for 3 s; Pierce strips barriers without the ×0.2 penalty. A button glows when its role counters what is on screen, and pulses red when it can break an incoming boss move.
- Rarity and footprint scale the hit (`RARITIES[].mul` from ×0.9 common to ×1.22 mythic, `SIZE_EDGE` from ×0.92 for 1×1 to ×1.12 for 2×2), both centred on 1, so a bigger or rarer piece earns its bag space. Mana is priced by the base hit (`damage / 2.6`, half-mana steps).
- Upgrades have diminishing returns: a whole press (all shots) hits ×1 / ×2.2 / ×3.5 / ×5.3 at levels 1–4 (`LEVEL_POWER`), while mana per press rises ×1 / ×1.3 / ×1.6 / ×2 (`MANA_LEVEL`). Level 4 is strong but no longer one-shots a level. Mana regenerates 14/s (+3 per Flow); after 1.5 s without attacking, **Focus** raises regen ×1.8 (the mana bar brightens). Kills give 5 mana (elites 10, bosses 0). Spamming drains the pool; waiting and then unloading a counter wins.
- Bosses fight back. Each has a signature rotation (`BOSS_SIGNATURES`): **SMASH** (a hit on the frog from anywhere for ×2.2 of the boss's power), **BROOD** (summons 3 adds) and **MEND** (heals 10% unless blighted). Before each one the boss winds up for 1.8 s with a ring, a timer and `… INCOMING · BREAK IT`. Dealing 9% of its max HP during the wind-up (Burst counts double, Vortex slows the timer) cancels the move and **staggers** the boss for 2.4 s: it stops acting and takes ×1.3 damage. At 66% and 33% HP the boss enters a new phase: it raises a barrier (Pierce breaks it), summons two adds and signs moves faster.
- Only the best copy of each weapon type attacks. Weaker copies are marked **SPARE** in the backpack and on loot cards: keep them to merge, or discard them for space. The item card shows the hit, mana, cooldown and damage per mana.
